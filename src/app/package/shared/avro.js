                    ...
                    if (typeof pattern.u !== "undefined") {
                        for (var j = 0; j < pattern.u.length; ++j) {
                            ...
                            for (var k = 0; k < rule.m.length; ++k) {
                                var match = rule.m[k];
                                if (match.t === "s") { chk = end } else { chk = prev }
                                ...
                                if (match.s === "p") { if (!(chk < 0 && match.t === "p" || chk >= fixed.length && match.t === "s" || this.isPunctuation(fixed.charAt(chk))) ^ match.negative) { r = false; break } } else if (match.s === "v") { if (!((chk >= 0 && match.t === "p" || chk < fixed.length && match.t === "s") && this.isVowel(fixed.charAt(chk))) ^ match.negative) { r = false; break } } else if (match.s === "c") { if (!((chk >= 0 && match.t === "p" || chk < fixed.length && match.t === "s") && this.isConsonant(fixed.charAt(chk))) ^ match.negative) { r = false; break } } else if (match.s === "e") {
                                    ...
                                    if (!this.isExact(match.v, fixed, s, e, match.negative)) { r = false; break }
                                }
                            }
                            if (r) {
                                output += rule.r;
...
//             })
//         },
//         notify: function(e) { this.callback(this.bangla) },
//         switchKb: function(e, state) {
//             if (typeof state === "undefined") { state = !this.bangla }
//             this.bangla = state;
//         },
//         focus: function(e) {
//             $(this).trigger("notify");
//             let avimode = localStorage.getItem('languageChange');
//             if (avimode == "Bangla") {
...
//             }
//         },
//         destroy: function() { return this.each(function() { $(this).unbind(".avro") }) },
//         keydown: function(e) {
//             var keycode = e.which;
...
//             if (el.selectionStart) { return el.selectionStart } else if (document.selection) {
//                 el.focus();
...
//         }
//     };
//     $.fn.avro = function(method) { if (method in ["init", "destroy"]) { return methods[method].apply(this, Array.prototype.slice.call(arguments, 1)) } else if (typeof method === "object" || !method) { return methods.init.apply(this, arguments) } else { $.error("Method " + method + " does not exist on jQuery.avro") } }
// })(jQuery);

console.log(OmicronLab.Avro.Phonetic);